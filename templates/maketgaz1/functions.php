...
if (!defined('_ARTX_FUNCTIONS')) {
    ...
    /**
     * Decorates an article or block with the gaz-post style.
     *
    ...
    function artxPost($data)
    {
        ...
        $classes = isset($data['classes']) && strlen($data['classes']) ? $data['classes'] : '';
                    artxFragmentBegin("<article class=\"gaz-post" . $classes . "\">");
            artxFragmentBegin("<h2 class=\"gaz-postheader\">");
            ...
            artxFragmentEnd("</h2>");
            artxFragmentBegin("<div class=\"gaz-postmetadataheader\">");
            artxFragmentBegin("<div class=\"gaz-postheadericons gaz-metadata-icons\">");
            ...
            artxFragmentEnd("</div>");
            artxFragmentBegin("<div class=\"gaz-postcontent clearfix\">");
    ...
    function artxBlock($caption, $content, $classes = '')
    {
        ...
        if (!$hasCaption && !$hasContent)
            return '';
                    artxFragmentBegin("<div class=\"gaz-block clearfix" . $classes . "\">");
            artxFragmentBegin("<div class=\"gaz-blockheader\"><h3 class=\"t\">");
            artxFragmentContent($caption);
            artxFragmentEnd("</h3></div>");
            artxFragmentBegin("<div class=\"gaz-blockcontent\">");
    ...
    function artxVMenuBlock($caption, $content, $classes = '')
{
    ...
    if (!$hasCaption && !$hasContent)
        return '';
                artxFragmentBegin("<div class=\"gaz-vmenublock clearfix" . $classes . "\">");
            artxFragmentBegin("<div class=\"gaz-vmenublockheader\"><h3 class=\"t\">");
            artxFragmentContent($caption);
            artxFragmentEnd("</h3></div>");
            artxFragmentBegin("<div class=\"gaz-vmenublockcontent\">");
            artxFragmentContent(artxPostprocessBlockContent($content));
            artxFragmentEnd("</div>");
     ...
     *  Select inputs with class != 'gaz-button'
     *   artxFindTags($html, array('input' => array('class' => '!gaz-button')))
     *  Select inputs with non-empty class
     *   artxFindTags($html, array('input' => array('class' => '!')))
     *  Select inputs with class != 'gaz-button' and non-empty class:
     *   artxFindTags($html, array('input' => array('class' => array('!gaz-button', '!'))))
     *  Select inputs with class = button but != 'gaz-button'
     *   artxFindTags($html, array('input' => array('class' => array('button', '!gaz-button'))))
     *
    ...
    function artxReplaceButtons($content, $filter = array('input' => array('class' => 'button')))
    {
        ...
        foreach (artxFindTags($content, $filter) as $tag) {
            if (is_string($tag))
                ...
                $tag['attributes']['class']['value'][] = 'gaz-button';
    ...
    function artxLinkButton($data = array())
    {
        return '<a class="' . (isset($data['classes']) && isset($data['classes']['a']) ? $data['classes']['a'] . ' ' : '')
            . 'gaz-button" href="' . $data['link'] . '">' . $data['content'] . '</a>';
    }
    ...
    {
        $content = artxReplaceButtons($content, array('input' => array('class' => array('button', '!gaz-button')),
            'button' => array('class' => array('button', '!gaz-button'))));
        return $content;
    }
}
