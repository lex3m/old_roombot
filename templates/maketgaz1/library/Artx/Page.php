...
class ArtxPage
{
    ...
    public function positions($positions, $style)
    {
        ...
        if (count($cells) == 0)
            return '';
        $result = '<div class="gaz-content-layout">';
        $result .= '<div class="gaz-content-layout-row">';
        foreach ($cells as $name => $width)
            $result .='<div class="gaz-layout-cell' . ('gaz-block' == $style ? ' gaz-layout-sidebar-bg' : '')
     ...
     *
     * The componentWrapper method gets the content of the 'component' buffer and searches for the '<div class="gaz-post">' string in it.
     * Then it wraps the content of the buffer with the gaz-post.
     */
    public function componentWrapper()
    {
        ...
        if ('com_content' == $option && ('featured' == $view || 'article' == $view || ('category' == $view && 'blog' == $layout)))
            return;
        if (false === strpos($content, '<div class="gaz-post'))
        ...
        CKEDITOR.on('instanceReady', function (evt) {
            ...
            if (includesTemplateStyle || includesEditorStyle) {
                ...
                if (includesTemplateStyle)
                    jQuery('body', ifrdoc).attr('id', 'gaz-main').addClass('gaz-postcontent');
            }
        });
    }
});
