...
if (!defined('_ARTX_FUNCTIONS')) {
    ...
    /**
     * Decorates an article or block with the gazpost style.
     *
    ...
    function artxPost($data)
    {
        ...
        $classes = isset($data['classes']) && strlen($data['classes']) ? $data['classes'] : '';
                    artxFragmentBegin("<article class=\"gazpost" . $classes . "\">");
            artxFragmentBegin("<h2 class=\"gazpostheader\">");
            ...
            artxFragmentEnd("</h2>");
            artxFragmentBegin("<div class=\"gazpostheadericons gazmetadata-icons\">");
            if (isset($data['metadata-header-icons']) && count($data['metadata-header-icons']))
                foreach ($data['metadata-header-icons'] as $icon)
                    artxFragment('', $icon, '', ' | ');
            artxFragmentEnd("</div>");
            artxFragmentBegin("<div class=\"gazpostcontent clearfix\">");
    ...
    function artxBlock($caption, $content, $classes = '')
    {
        ...
        if (!$hasCaption && !$hasContent)
            return '';
                    artxFragmentBegin("<div class=\"gazblock clearfix" . $classes . "\">");
            artxFragmentBegin("<div class=\"gazblockheader\"><h3 class=\"t\">");
            artxFragmentContent($caption);
            artxFragmentEnd("</h3></div>");
            artxFragmentBegin("<div class=\"gazblockcontent\">");
    ...
    function artxVMenuBlock($caption, $content, $classes = '')
{
    ...
    if (!$hasCaption && !$hasContent)
        return '';
                artxFragmentBegin("<div class=\"gazvmenublock clearfix" . $classes . "\">");
            artxFragmentBegin("<div class=\"gazvmenublockheader\"><h3 class=\"t\">");
            artxFragmentContent($caption);
            artxFragmentEnd("</h3></div>");
            artxFragmentBegin("<div class=\"gazvmenublockcontent\">");
            artxFragmentContent(artxPostprocessBlockContent($content));
            artxFragmentEnd("</div>");
     ...
     *  Select inputs with class != 'gazbutton'
     *   artxFindTags($html, array('input' => array('class' => '!gazbutton')))
     *  Select inputs with non-empty class
     *   artxFindTags($html, array('input' => array('class' => '!')))
     *  Select inputs with class != 'gazbutton' and non-empty class:
     *   artxFindTags($html, array('input' => array('class' => array('!gazbutton', '!'))))
     *  Select inputs with class = button but != 'gazbutton'
     *   artxFindTags($html, array('input' => array('class' => array('button', '!gazbutton'))))
     *
    ...
    function artxReplaceButtons($content, $filter = array('input' => array('class' => 'button')))
    {
        ...
        foreach (artxFindTags($content, $filter) as $tag) {
            if (is_string($tag))
                ...
                $tag['attributes']['class']['value'][] = 'gazbutton';
    ...
    function artxLinkButton($data = array())
    {
        return '<a class="' . (isset($data['classes']) && isset($data['classes']['a']) ? $data['classes']['a'] . ' ' : '')
            . 'gazbutton" href="' . $data['link'] . '">' . $data['content'] . '</a>';
    }
    ...
    {
        $content = artxReplaceButtons($content, array('input' => array('class' => array('button', '!gazbutton')),
            'button' => array('class' => array('button', '!gazbutton'))));
        return $content;
    }
}
