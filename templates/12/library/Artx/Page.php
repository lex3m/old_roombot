...
class ArtxPage
{
    ...
    public function positions($positions, $style)
    {
        ...
        if (count($cells) == 0)
            return '';
        $result = '<div class="gazcontent-layout">';
        $result .= '<div class="gazcontent-layout-row">';
        foreach ($cells as $name => $width)
            $result .='<div class="gazlayout-cell' . ('gazblock' == $style ? ' gazlayout-sidebar-bg' : '')
     ...
     *
     * The componentWrapper method gets the content of the 'component' buffer and searches for the '<div class="gazpost">' string in it.
     * Then it wraps the content of the buffer with the gazpost.
     */
    public function componentWrapper()
    {
        ...
        if ('com_content' == $option && ('featured' == $view || 'article' == $view || ('category' == $view && 'blog' == $layout)))
            return;
        if (false === strpos($content, '<div class="gazpost'))
        ...
        CKEDITOR.on('instanceReady', function (evt) {
            ...
            if (includesTemplateStyle || includesEditorStyle) {
                ...
                if (includesTemplateStyle)
                    jQuery('body', ifrdoc).attr('id', 'gazmain').addClass('gazpostcontent');
            }
        });
    }
});
