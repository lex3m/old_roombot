                    ...
                    <?php foreach ($data->taglinks as $m) {
                        $tag=Mobiletags::model()->findByPk($m->tagId);?>
                         <div style="margin-bottom:10px;" id="taglinkk<?php echo $m->id; ?>">
                             <?php echo $tag->name; ?>
                             <?php if (Yii::app()->user->id == $member->id): ?>
                                <a href="<?php echo Yii::app()->createUrl('/mobilepictures/delete'); ?>" target="<?php echo $m->id; ?>" id="delete_tag" title="Удалить"><img width="12px" height="12px" src="<?php echo Yii::app()->baseUrl; ?>/images/site/delete_icon.png" style="float:right" id="<?php echo $m->id; ?>" /></a>
                             <?php endif; ?>
                         </div>
                    <?php } ?>
                 </div>
