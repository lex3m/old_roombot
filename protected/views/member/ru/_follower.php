        ...
        <?php if (count($data->followed->comments) > 0): ?>
            <?php foreach($data->followed->comments as $comment): ?>
                  <div id="<?php echo $comment->id; ?>" class="commentBodyContent">
                      ...
                      <a class="rb-username"  href="<?php echo Yii::app()->createUrl('member/dashboard',array('id'=>$comment->member->urlID)); ?>" data-type="profile"><?php echo $comment->member->login; ?></a> прокомментировал(-а) <?php echo CHtml::link('фотографию', array('mobilepictures/viewinfo', 'id'=>$comment->photoID), array('target'=>'blank')) ;?>
