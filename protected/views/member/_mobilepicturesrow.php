...
<div class="user-photo" name="<?php echo $data->id; ?>">
    <div class="info-userfoto">
     ...
     <div class="daty"><?php echo $data->date; ?>
         ...
         no comments
         <?php } else { ?>
         <a  target="_blank" href="<?php echo Yii::app()->createUrl('mobilepictures/viewinfo',array('id'=>$data->id)); ?>"><?php echo Yii::t('app', '{n} comment|{n} comments', $data->countComments);  ?></a>
         <?php } ?>
    </div>

    <div id="company_photo" style="margin-bottom:10px;" class="" >
             <?php if (Yii::app()->user->id == $member->id): ?>
                <a href="<?php echo Yii::app()->createUrl('mobilepictures/viewinfo',array('id'=>$data->id)); ?>" target="<?php echo $data->id; ?>" id="rotate_picture" title="Rotate photo"><img width="15px" height="15px" src="<?php echo Yii::app()->baseUrl; ?>/images/rotate.png" style="float:left; margin-right: 5px;"></a>
             <?php endif ;?>
             <a id="img-<?php echo $data->id; ?>" href="<?php echo Yii::app()->createUrl('mobilepictures/viewinfo',array('id'=>$data->id)); ?>" target="_blank">
            ...
            <div style="margin-left:20px;" class="izo-tegi">
                <span class="tegss">Description:</span> <span id="picture_info" data-id="<?php echo $data->id; ?>" ><?php if (Yii::app()->user->id == $member->id): ?><img class="buttonsCommentAction buttonCommentEditIcon" style="padding-top: 0px; margin-top: -5px; cursor:pointer;" src=""></span><?php endif;?>
                    ...
                    <?php foreach ($data->taglinks as $m) {
                        $tag=Mobiletags::model()->findByPk($m->tagId);?>
                         <div style="margin-bottom:10px;" id="taglinkk<?php echo $m->id; ?>">
                             ...
                             <?php if (Yii::app()->user->id == $member->id): ?>
                                <a href="<?php echo Yii::app()->createUrl('/mobilepictures/delete'); ?>" target="<?php echo $m->id; ?>" id="delete_tag" title="Delete"><img width="12px" height="12px" src="<?php echo Yii::app()->baseUrl; ?>/images/site/delete_icon.png" style="float:right" id="<?php echo $m->id; ?>" /></a>
                             <?php endif; ?>
                         </div>
                    <?php } ?>
                 </div>
             ...
             <?php if (Yii::app()->user->id == $member->id): ?>
             <div class="izo-tegi2">
                  <a href="#" id="<?php echo $data->id; ?>" name="new_tag" class="a-teg">Add tag</a>
             </div>
             <?php endif; ?>
     </div>
</div>
